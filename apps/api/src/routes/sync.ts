...
router.post('/characters', requireAuth, async (req, res): Promise<void> => {
  try {
    ...
    console.log(`[Sync API] POST /sync/characters - User: ${userEmail} (${userId})`);
    console.log(`[Sync API] Received ${characters?.length || 0} characters in request body`);

    const result = await syncService.syncCharacters(userId, characters);
    
    console.log(`[Sync API] Sync successful for user ${userEmail}`);
    console.log(`[Sync API] Sending response with ${result.characterCount} characters`);
...
router.get('/status', requireAuth, async (req, res): Promise<void> => {
  try {
    ...
    console.log(`[Sync API] Status retrieved for user ${userEmail}:`, {
      ...
      lastSyncedAt: status.lastSyncedAt ? new Date(status.lastSyncedAt).toISOString() : 'never',
...
router.delete('/characters/:characterId', requireAuth, async (req, res): Promise<void> => {
  try {
    ...
    console.log(`[Sync API] DELETE /sync/characters/${characterId} - User: ${userEmail} (${userId})`);

    await syncService.deleteCharacterBackup(userId, characterId);
    
    console.log(`[Sync API] Successfully deleted character ${characterId} for user ${userEmail}`);
