...
import { Sparkles, ChevronDown, ChevronRight, Lock, Unlock } from "lucide-react";
...
type FeatureWithSource = ClassFeatureWithSource | AncestryFeatureWithSource | BackgroundFeatureWithSource;
...
export function ClassFeaturesSection() {
  ...
  const allFeatures: FeatureWithSource[] = [
    ...
    ...ancestryFeatures.map((feature): AncestryFeatureWithSource => ({
      source: 'ancestry',
      feature
    })),
    ...backgroundFeatures.map((feature): BackgroundFeatureWithSource => ({
      source: 'background',
      feature
    }))
  ];
  ...
  const classSubclassFeatures = allFeatures.filter(f => 
    f.source === 'class' || f.source === 'subclass'
  ).sort((a, b) => {
          ...
          <div className="pt-2 border-t">
            {feature.effects
              ...
              .map((effect: any, effectIndex: number) => {
                ...
                const spellsByTier = spells.reduce((acc, spell) => {
                ...
                return (
                  <div key={effectIndex} className="space-y-2">
                    <Button
                      ...
                      onClick={() => setExpandedSpellSchools({
                            ...
                            return (
                              <div key={tier} className="space-y-2">
                                <div className="flex items-center gap-2">
                                  ...
                                  <Badge variant="outline" className={getTierColor(tierNum) + " text-xs"}>
                                ...
                                <div className="grid gap-2">
                                  {tierSpells.map(spell => (
                                    ...
                                    >
                                      ...
                                      <div className="text-muted-foreground">{spell.description}</div>
                                      {spell.resourceCost && (
                                        <div className="mt-1 text-blue-600">
                                          Cost: {spell.resourceCost.type === 'fixed' 
