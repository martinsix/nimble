  ...
  return (
    <>
      <div className="flex items-center space-x-2">
        ...
        <span className="text-muted-foreground">({formatRoll(roll)})</span>
              ...
              <div className="text-sm mt-1">
                {roll.dice.map((die, index) => (
                  <div
                    key={index}
                    className={
                      die.isCritical ? "text-yellow-600 font-semibold" : ""
                    }
                  >
  ...
  return (
    <>
      ...
      <div className="flex items-center space-x-2">
        ...
        <span
          className={`font-bold text-lg min-w-8 text-right ${getEntryColor()}`}
        >
          {entry.type === "damage"
            ? `-${entry.amount}`
            : entry.type === "healing"
            ...
            ? `${(entry as AbilityUsageEntry).usesRemaining}/${
                (entry as AbilityUsageEntry).maxUses
              }`
